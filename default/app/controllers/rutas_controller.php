...
class RutasController extends RestController
{
      ...
      {
            if ((new Rutas())->find($id)) {
                  $this->data = (new Rutas())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Rutas())->find($id)) {
                  $registro = (new Rutas())->find($id);
                  $registroActualizado = (new Rutas())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Rutas())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

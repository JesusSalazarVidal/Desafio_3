...
class PrestamosController extends RestController
{
      ...
      {
            if ((new Prestamos())->find($id)) {
                  $this->data = (new Prestamos())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Prestamos())->find($id)) {
                  $registro = (new Prestamos())->find($id);
                  $registroActualizado = (new Prestamos())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Prestamos())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

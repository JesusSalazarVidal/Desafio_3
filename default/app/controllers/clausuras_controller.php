...
class ClausurasController extends RestController
{
      ...
      {
            if ((new Clausuras())->find($id)) {
                  $this->data = (new Clausuras())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Clausuras())->find($id)) {
                  $registro = (new Clausuras())->find($id);
                  $registroActualizado = (new Clausuras())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Clausuras())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

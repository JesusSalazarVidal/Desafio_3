...
class HorariosController extends RestController
{
      ...
      {
            if ((new Horarios())->find($id)) {
                  $this->data = (new Horarios())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Horarios())->find($id)) {
                  $registro = (new Horarios())->find($id);
                  $registroActualizado = (new Horarios())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Horarios())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

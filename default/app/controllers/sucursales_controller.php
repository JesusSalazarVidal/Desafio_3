...
class SucursalesController extends RestController
{
      ...
      {
            if ((new Sucursales())->find($id)) {
                  $this->data = (new Sucursales())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Sucursales())->find($id)) {
                  $registro = (new Sucursales())->find($id);
                  $registroActualizado = (new Sucursales())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Sucursales())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

...
class AgenciasController extends RestController
{
      ...
      {
            if ((new Agencias())->find($id)) {
                  $this->data = (new Agencias())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Agencias())->find($id)) {
                  $registro = (new Agencias())->find($id);
                  $registroActualizado = (new Agencias())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Agencias())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

...
class DescuentosController extends RestController
{
      ...
      {
            if ((new Descuentos())->find($id)) {
                  $this->data = (new Descuentos())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Descuentos())->find($id)) {
                  $registro = (new Descuentos())->find($id);
                  $registroActualizado = (new Descuentos())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Descuentos())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

...
class AutomovilesController extends RestController
{
      ...
      {
            if ((new Automoviles())->find($id)) {
                  $this->data = (new Automoviles())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Automoviles())->find($id)) {
                  $registro = (new Automoviles())->find($id);
                  $registroActualizado = (new Automoviles())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Automoviles())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

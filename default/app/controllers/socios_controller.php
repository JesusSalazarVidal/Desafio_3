...
class SociosController extends RestController
{
      ...
      {
            if ((new Socios())->find($id)) {
                  $this->data = (new Socios())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Socios())->find($id)) {
                  $registro = (new Socios())->find($id);
                  $registroActualizado = (new Socios())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Socios())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

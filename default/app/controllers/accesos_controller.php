...
class AccesosController extends RestController
{
      ...
      {
            if ((new Accesos())->find($id)) {
                  $this->data = (new Accesos())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Accesos())->find($id)) {
                  $registro = (new Accesos())->find($id);
                  $registroActualizado = (new Accesos())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Accesos())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

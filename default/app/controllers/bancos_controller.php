...
class BancosController extends RestController
{
      ...
      {
            if ((new Bancos())->find($id)) {
                  $this->data = (new Bancos())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Bancos())->find($id)) {
                  $registro = (new Bancos())->find($id);
                  $registroActualizado = (new Bancos())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Bancos())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

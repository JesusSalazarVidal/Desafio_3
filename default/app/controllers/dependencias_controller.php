...
class DependenciasController extends RestController
{
      ...
      {
            if ((new Dependencias())->find($id)) {
                  $this->data = (new Dependencias())->find($id);
            } else {
      ...
      public function put()
      {
            ...
            if ($data) {
                  $nuevoRegistro = new Dependencias();
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Dependencias())->find($id)) {
                  $registro = (new Dependencias())->find($id);
                  $registroActualizado = (new Dependencias())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Dependencias())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

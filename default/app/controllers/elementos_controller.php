...
class ElementosController extends RestController
{
      ...
      {
            if ((new Elementos())->find($id)) {
                  $this->data = (new Elementos())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Elementos())->find($id)) {
                  $registro = (new Elementos())->find($id);
                  $registroActualizado = (new Elementos())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Elementos())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

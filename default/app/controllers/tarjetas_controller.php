...
class TarjetasController extends RestController
{
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Tarjetas())->find($id)) {
                  $registro = (new Tarjetas())->find($id);
                  $registroActualizado = (new Tarjetas())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Tarjetas())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

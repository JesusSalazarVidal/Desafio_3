...
class CiudadesController extends RestController
{
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Ciudades())->find($id)) {
                  $registro = (new Ciudades())->find($id);
                  $registroActualizado = (new Ciudades())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Ciudades())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

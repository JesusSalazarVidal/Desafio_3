...
class FormulasController extends RestController
{
      ...
      {
            if ((new Formulas())->find($id)) {
                  $this->data = (new Formulas())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Formulas())->find($id)) {
                  $registro = (new Formulas())->find($id);
                  $registroActualizado = (new Formulas())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Formulas())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

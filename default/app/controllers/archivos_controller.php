...
class ArchivosController extends RestController
{
      ...
      {
            if ((new Archivos())->find($id)) {
                  $this->data = (new Archivos())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Archivos())->find($id)) {
                  $registro = (new Archivos())->find($id);
                  $registroActualizado = (new Archivos())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Archivos())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

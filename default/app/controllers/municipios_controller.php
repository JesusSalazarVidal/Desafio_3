...
class MunicipiosController extends RestController
{
      ...
      {
            if ((new Municipios())->find($id)) {
                  $this->data = (new Municipios())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Municipios())->find($id)) {
                  $registro = (new Municipios())->find($id);
                  $registroActualizado = (new Municipios())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Municipios())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

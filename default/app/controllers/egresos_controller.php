...
class EgresosController extends RestController
{
      ...
      {
            if ((new Egresos())->find($id)) {
                  $this->data = (new Egresos())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Egresos())->find($id)) {
                  $registro = (new Egresos())->find($id);
                  $registroActualizado = (new Egresos())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Egresos())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }

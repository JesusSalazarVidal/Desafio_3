...
class ContadoresController extends RestController
{
      ...
      {
            if ((new Contadores())->find($id)) {
                  $this->data = (new Contadores())->find($id);
            } else {
      ...
      public function patch($id)
      {
            $data = json_decode(file_get_contents('php://input'));
            if ((new Contadores())->find($id)) {
                  $registro = (new Contadores())->find($id);
                  $registroActualizado = (new Contadores())->find($id);
                  foreach ($data as $key => $dato) {
      ...
      {
            $this->data = (new Contadores())->paginate("page: $page", "per_page: $ppage", 'order: id asc');
      }
